module "prestage_thumbimg_event_rule" {
  ...
  target_arn      = local.enclave_thubmnail_queue1_arn
  event_rule_role_arn = concat(module.eventrule_roles_policies.*.role_arn, [""])[0]
  tags            = local.tags
  #depends_on      = [module.thubmnail_sqs_queue1]
}
...
module "datasync_thumbimg_event_rule1" {
  count              = local.create_endor_thumbimg_infra ? 1 : 0
  source             = "../terraform-modules/common-modules/terraform-aws-eventbridge"
...
module "datasync_thumbimg_event_rule2" {
  count              = local.create_endor_thumbimg_infra ? 1 : 0
  source             = "../terraform-modules/common-modules/terraform-aws-eventbridge"
